...
def with_timeout(
    parent: CancellationToken, timeout: float
) -> tuple[CancellationToken, Callable[[], None]]:
    ...
    timer = threading.Timer(
        timeout, partial(token.cancel, CancellationTokenTimeoutError())
    )
...
def is_token_cancelled(token: CancellationToken) -> TypeGuard[CancelledToken]:
...
class _CancellableToken:
    ...
    def cancel(self, error: Exception | None = None) -> None:
        with self.__lock:
            ...
            self.__error = error or CancellationTokenCancelledError()
